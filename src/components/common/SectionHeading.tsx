...
const subtitleVariants = cva('line-clamp-1', {
  variants: {
    variant: {
      default: 'mb-1 font-semibold text-primary uppercase tracking-wider',
      gradient:
        'bg-clip-text bg-linear-to-r from-primary to-primary/70 font-bold text-transparent uppercase tracking-widest',
      underline:
        'inline-block relative font-bold text-primary uppercase tracking-widest after:absolute after:bottom-0 after:left-0 after:bg-primary after:rounded-full after:w-full after:h-0.5',
      badge:
        'inline-flex items-center gap-2 bg-linear-to-r from-primary/10 via-primary/5 to-transparent px-4 py-2 rounded-full border border-primary/20 shadow-sm shadow-primary/10 font-semibold text-primary text-sm uppercase tracking-wider'
    },
    animated: {
      true: 'animate-in fade-in slide-in-from-bottom-2 duration-500',
      false: ''
    ...
    <div className={cn(sectionHeadingVariants({ alignment }), className)}>
      ...
      {subtitle && (
        <div className={cn(subtitleWrapperVariants({ variant, animated: animated }))}>
          ...
          {variant === 'default' ? (
            ...
            >
              {hasBadgeDot && (
                <span className='bg-primary rounded-full w-1.5 h-1.5 animate-pulse' />
              )}
              {subtitle}
            </Typography>
          )}
      ...
      {description && (
        <Typography
          variant='body1'
          className={cn(
            descriptionVariants({ alignment, animated: animated }),
            descriptionClassName
          )}
