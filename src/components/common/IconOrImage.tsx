...
export interface IconOrImageProps extends VariantProps<typeof iconOrImageVariants> {
    ...
    strokeWidth?: number
    color?: 'default' | 'primary' | 'secondary' | 'success' | 'warning' | 'danger' | 'muted' | 'white' | 'inherit'
}
...
export function IconOrImage({
    ...
    const elementClasses = cn(
        iconOrImageVariants({ size, variant: variant === 'contained' || variant === 'gradient' ? 'default' : variant }),
        iconClassName
    )

    // Render Lucide Icon
    if (isLucideIcon(icon)) {
        const iconName = (icon in LucideIcons ? icon : icon.charAt(0).toUpperCase() + icon.slice(1)) as keyof typeof LucideIcons
        ...
        return (
            <div className={wrapperClasses}>
                <IconComponent
                    className={cn(elementClasses, iconColorClass)}
