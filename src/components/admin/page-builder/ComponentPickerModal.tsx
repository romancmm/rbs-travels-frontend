...
export function ComponentPickerModal({
  ...
  const handleSelectComponent = (componentType: string) => {
    ...
    // If custom callback provided, use it (for grid items, etc.)
    if (onSelectComponent) {
      onSelectComponent(componentType)
    } else if (columnId) {
      // Otherwise use default behavior for columns
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='flex flex-col max-w-3xl max-h-[80vh] overflow-hidden'>
        <DialogHeader>
          ...
          <DialogDescription>
            Choose a component to add. You can search or browse by category.
          </DialogDescription>
        </DialogHeader>
