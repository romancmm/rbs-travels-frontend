...
export function PageBuilder({ pageId, initialContent, pageMetadata }: PageBuilderProps) {
  // Store hooks
  ...
  // Handlers for save/publish with error handling
  ...
  useEffect(() => {
    if (initialContent) {
      loadPage(pageId, initialContent, pageMetadata)
    }
  }, [pageId, initialContent, pageMetadata, loadPage])
      ...
      <div className='flex flex-col bg-gray-50 h-screen'>
        ...
        <div className='flex justify-between items-center bg-white shadow-sm px-4 border-b h-14'>
          ...
          <div className='flex items-center gap-4'>
            ...
            {isDirty && <span className='text-amber-600 text-xs'>Unsaved changes</span>}
          </div>
          ...
          <div className='flex items-center gap-1 bg-gray-50 p-1 border rounded-lg'>
            {previewModes.map(({ mode, icon: Icon, label }) => (
              <Button
                ...
                className={cn('gap-2 h-8', previewMode === mode && 'bg-white shadow-sm')}
              >
