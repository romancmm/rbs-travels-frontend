  ...
  return (
    <div className='space-y-4'>
      ...
      <Tabs
        ...
        onValueChange={(val: any) => handleChange({ ...config, type: val as any })}
      >
        ...
        <TabsContent value='color' className='space-y-4 pt-4'>
          <div className='space-y-2'>
            ...
            <Select
              ...
              onValueChange={(val) =>
                handleChange({ ...config, type: 'color', color: val === 'none' ? undefined : val })
              }
            >
          ...
          <div className='space-y-2'>
            ...
            <p className='text-[10px] text-muted-foreground'>
              Use any Tailwind color class or arbitrary value
            </p>
          </div>
        </TabsContent>
        ...
        <TabsContent value='gradient' className='space-y-4 pt-4'>
              ...
              <SelectContent>
                {GRADIENT_DIRECTIONS.map((dir) => (
                  <SelectItem key={dir.value} value={dir.value}>
                    <div className='flex items-center gap-2'>
                      <div
                        className={`w-4 h-4 rounded bg-gradient-${dir.value} from-blue-400 to-purple-600`}
                      />
                      {dir.label}
                    </div>
                  </SelectItem>
          ...
          {(config.gradient?.from || config.gradient?.to) && (
            <div className='bg-muted/30 mt-4 p-4 border rounded-lg'>
              <Label className='block mb-2 font-medium text-muted-foreground text-xs'>
                Preview
              </Label>
              <div
                className={`h-20 rounded-md bg-gradient-${config.gradient?.direction || 'to-r'} ${
                  config.gradient?.from ? `from-${config.gradient.from}` : ''
                } ${config.gradient?.via ? `via-${config.gradient.via}` : ''} ${
                  config.gradient?.to ? `to-${config.gradient.to}` : ''
                } shadow-md`}
              />
            </div>
          )}
        ...
        <TabsContent value='image' className='space-y-4 pt-4'>
          <div className='space-y-2'>
            ...
            />
            <p className='text-[10px] text-muted-foreground'>
              Enter the path to your background image
            </p>
          </div>
          ...
          <div className='bg-amber-50 dark:bg-amber-950/20 p-3 border border-amber-200 dark:border-amber-800 rounded-lg'>
            <p className='text-[11px] text-amber-800 dark:text-amber-200 leading-relaxed'>
              <strong>Note:</strong> Background images are applied via inline styles (not Tailwind
              classes) to support dynamic URLs.
            </p>
          </div>
        </TabsContent>
      </Tabs>
      ...
      <div className='space-y-2'>
        ...
        <div className='bg-muted/50 p-3 border border-muted rounded-lg'>
          <code className='font-mono text-foreground text-xs break-all'>
            {generateBackgroundClasses(config) || (
              <span className='text-muted-foreground italic'>No classes generated yet</span>
            )}
          </code>
        </div>
      </div>
