...
function DraggableComponentItem({ component }: DraggableComponentItemProps) {
  ...
  const [dragId] = useState(
    () => `new-${component.type}-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`
  )
  ...
  return (
    <div className='flex flex-col bg-background border-r w-[280px] h-full'>
      ...
      <div className='space-y-2 p-4 border-t shrink-0'>
        ...
        <p className='text-[10px] text-muted-foreground'>Drop onto columns to add new components</p>
      </div>
    </div>
  )
}
