...
import WhoWeAreSection from '@/components/admin/form/settings/home/WhoWeAre'
...
export default function WhoWeAreConfigPage() {
  ...
  return (
    <>
      <PageHeader
        title='Who We Are'
        subTitle='Manage the who we are section displayed on the homepage'
        extra={
      ...
      {edit ? (
        <WhoWeAreSection settingsKey={settingsKey} initialValues={homeConfig} refetch={onClose} />
      ) : loading ? (
        Array.from({ length: 2 }).map((_, idx) => <Skeleton className='my-8' key={idx} />)
      ) : homeConfig ? (
        <div className='space-y-6'>
          {homeConfig.whoWeAre ? (
            <RenderData data={homeConfig?.whoWeAre} excludedFields={[]} />
          ) : (
            <EmptyState />
          )}
        </div>
